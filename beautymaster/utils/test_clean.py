...
def test_single(image_path):
    prompt = f"你是一位火眼晶晶的数据筛选工程师，我们的目标是找到出那些只有一个完整人的全身照，你需要做的工作是将不需要的图片意义删选出来，并给出0或1，请仔细分析我们提供给你的图片，不需要的图片包含以下几个特征：1.没有完整的脸；2.不是站立的姿势；3.不是全身照，没有从头人到脚的是一个完整的人；4.一张图片里面包含多个人；5.图片模糊不清，人的占比很小；6.没有人出现；。以上是不需要的图片的特征，如果图片含有以上5个特征中的一个请输出1，如果没有请输出0，请简要的说明理由。"
...
prompt = f"""你是一位火眼晶晶的数据筛选工程师，本次数据筛选的目标是找到出那些只有一个完整人的全身照,你需要做的工作是将需要的图片筛选选出来，请仔细分析我们提供给你的图片，需要的图片必须满足以下5个特征：
